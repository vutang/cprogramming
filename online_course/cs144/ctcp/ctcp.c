...
#define WAIT_FINISH       IDLE << 2
#define SEND_FIN          IDLE << 3
#define GOT_FIN           IDLE << 4
#define FINISH            IDLE << 5
...
void ctcp_read(ctcp_state_t *state) {
  if (((state->status & IDLE) | !(state->status & WAIT_ACK)) && !(state->status & WAIT_FINISH)) {
...
void ctcp_send_ack(ctcp_state_t *state, ctcp_segment_t *segment, uint32_t fin) {
  ...
  /*Build up ACK*/
  make_segment(segment_to_send, state->seqno, ackno, 0, ACK, segment->window, NULL);
  /*Send ACK segment*/
...
void handle_retransmittion(ctcp_state_t *state) {
  ...
  if ((state->retransmit_time < 5) && ((state->status & (WAIT_ACK)))) {
    ...
    if (state->retransmit_time >= 5)
      fprintf(stderr, "[Warning] Over retrans_times, call destroy state %d\n", state->number_in_list);
      fprintf(stderr, "[Info] Teardown!!!%d\n", state->number_in_list);
    ctcp_destroy(state);
  }      
  return;
}
...
void handle_fin(ctcp_state_t *state) {
  ...
  else if ((state->status & WAIT_FINISH) && !(state->status & WAIT_ACK)) {
    if (start_timer == 0) 
      start_timer = 1;
    else {
      ...
      if (ctcp_send_fin(state) < 0)
        return;
      state->status = (SEND_FIN | WAIT_ACK);
      start_timer = 0;
    }
  }
  /*Get FIN, and SEND FIN back*/
  else if (state->status & GOT_FIN) {
    if ((state->status & SEND_FIN)) {
      if (!(state->status & WAIT_ACK)) {
        if (start_timer == 0) 
          start_timer = 1;
