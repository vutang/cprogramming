...
class CS144Topo( Topo ):
	...
	for h in server1, server2, server3, nat:
...
def set_default_route_client(host):
    ...
    for eth in ['sw0-eth1', 'server1', 'server2', 'server3']:
        swip = IP_SETTING[eth]
...
def cs144net():
    ...
    net.start()
    server1, server2, server3, client, router = net.get( 'server1', 'server2', 'client', 'sw0')
    ...
    s2intf = server3.defaultIntf()
    s2intf.setIP('%s/8' % IP_SETTING['server3'])
