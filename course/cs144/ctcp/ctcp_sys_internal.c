...
int conn_input(conn_t *conn, void *buf, size_t len) { ASSERT_CONN;
  ...
  if (conn->read_eof) {
    return -1;
    fprintf(stderr, "[INFO] Get an EOF signal\n");
  }
...
conn_t *tcp_new_connection(char *pkt) { ASSERT_SERVER_ONLY;
  ...
  fprintf(stderr, "[INFO] Client connected, current conn number: %d\n", num_connected);
  return conn;
}
...
void execute_program(conn_t *conn) { ASSERT_SERVER_ONLY;
  ...
  fprintf(stderr, "[INFO] Execute program: %s %d\n", config->program, config->argc);
  ...
  if (fork() == 0) {
    ...
    // while ((read(CHILD_READ_FD, buf, MAX_SEG_DATA_SIZE)) > 0) {
...
void print_conn(conn_t *conn) {
  ...
  memcpy(&conn_cpy, conn, sizeof(conn_t));
  memcpy(&ip_addr_tmp,&conn_cpy.ip_addr,sizeof(struct in_addr));

  printf("[vINFO] Connection information:\n");
  printf("\t\tip_addr: %s\n", inet_ntoa(ip_addr_tmp));
  printf("\t\tport: %d\n", conn_cpy.port);
  printf("\t\tseqno: %d\n", conn_cpy.seqno);
  printf("\t\tnext_seqno: %d\n", conn_cpy.next_seqno);
  printf("\t\tackno: %d\n", conn_cpy.ackno);
