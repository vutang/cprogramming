/*
* @Author: vutang
* @Date:   2019-05-10 10:01:11
* @Last Modified by:   vutang
* @Last Modified time: 2019-05-10 10:15:10
*/
/******************************************************************************
 *
...
reactor_t *reactor_new(void) {
	...
	if (ret->epoll_fd == INVALID_FD) {
		LOG_ERROR("%s unable to create epoll instance: %s", 
			__func__, strerror(errno));
		goto error;
	}
	ret->event_fd = eventfd(0, 0);
	if (ret->event_fd == INVALID_FD) {
		LOG_ERROR("%s unable to create eventfd: %s", 
			__func__, strerror(errno));
		goto error;
	}
	...
	if (!ret->invalidation_list) {
		LOG_ERROR("%s unable to allocate object invalidation list.", 
			__func__);
		goto error;
	}
	...
	if (epoll_ctl(ret->epoll_fd, EPOLL_CTL_ADD, ret->event_fd, &event) == -1) {
		LOG_ERROR("%s unable to register eventfd with epoll set: %s", 
			__func__, strerror(errno));
		goto error;
	}
	return ret;
...
reactor_object_t *reactor_register(reactor_t *reactor, int fd, void *context,
	void (*read_ready)(void *context), void (*write_ready)(void *context)) 
{
	assert(reactor != NULL);
	...
	if (!object) {
		LOG_ERROR("%s unable to allocate reactor object: %s", 
			__func__, strerror(errno));
		return NULL;
	}
	...
	if (epoll_ctl(reactor->epoll_fd, EPOLL_CTL_ADD, fd, &event) == -1) {
		LOG_ERROR("%s unable to register fd %d to epoll set: %s", 
...
bool reactor_change_registration(reactor_object_t *object,
	...
	if (epoll_ctl(object->reactor->epoll_fd, EPOLL_CTL_MOD, object->fd, &event) == -1) {
		LOG_ERROR("%s unable to modify interest set for fd %d: %s", 
...
void reactor_unregister(reactor_object_t *obj) {
	...
	if (epoll_ctl(reactor->epoll_fd, EPOLL_CTL_DEL, obj->fd, NULL) == -1)
		LOG_ERROR("%s unable to unregister fd %d from epoll set: %s", 
