...
void ctcp_destroy(ctcp_state_t *state) {
  ...
  *state->prev = state->next;
  ...
  fprintf(stderr, "\tRemoving connection by using conn_remove()....\n");
  conn_remove(state->conn);
  fprintf(stderr, "\tFreeing list of segmenst...\n");
...
void free_sent_segment(ctcp_state_t *state) {
  if (state->segments->head->next != NULL) {
    ...
    free(node_tmp);
    // while ((ntohl(data_node->seqno) < state->base_seqno - state->send_window) && (state->segments->head->next != NULL)) {
...
int ctcp_send(ctcp_state_t *state, uint32_t flags, char *buf, int len) {
  ...
  fprintf(stderr, "[vINFO] Sending data segment, base_seqno: %d, seqno: %d\n", 
...
void handle_fin_segment (ctcp_state_t *state, ctcp_segment_t *segment) {
  ctcp_send_ack(state, segment, TH_FIN);
...
int ctcp_send_fin (ctcp_state_t *state) {
  ...
  make_segment(segment, 0, 0, 0, TH_FIN, htons(MAX_SEG_DATA_SIZE), NULL);
  /*Send segment*/
...
/*Handling retransmittion when error occur*/
  ...
  if (((state->base_seqno == ntohl(state->received_ackno)) && (state->base_seqno < ntohl(state->seqno))) 
    ...
    if (start_timer == 0) 
      ...
      if (ctcp_send_fin(state) < 0) {
        fprintf(stderr, "Cannot send FIN again\n");
      }
      start_timer = 0;
    }
  }
